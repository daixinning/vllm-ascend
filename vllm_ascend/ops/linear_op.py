...
class SequenceColumnParallelOp(CustomColumnParallelOp):
    def apply_impl(self, input_: torch.Tensor) -> torch.Tensor | tuple[torch.Tensor, Parameter | None]:
        ...
        # Pre-AllGather DynamicMxQuant (FlashComm1 column half): quantize the local
        # shard to fp8 BEFORE the all_gather so the collective moves fp8 + E8M0
        # scale instead of bf16 -> ~half the column-path comm volume. Per-token
        # mx-quant is row-independent, so quantizing per shard row is numerically
        # identical to quantizing after the gather. mxfp8 + A5 + no-bias only.
