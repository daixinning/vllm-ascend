...
def _gdn_gating_rows(
    ...
    """Body of fused_gdn_gating_kernel for the rows owned by ``core_id``.

    Elementwise over tokens - only A_log/dt_bias are indexed by head and there is no
    reduction across tokens - so computing a token sub-range is bit-identical to
    computing the full range and slicing it. That is what lets gating join a
    prefill-only launch. ``seq_len`` is always 1 upstream, so the offset that reads
    ``i_s * NUM_HEADS`` there collapses away here.
    """
    COL_ITER = tl.cdiv(NUM_HEADS, BLK_HEADS)
    for row_idx in range(0, ROW_ITER):
        batch_off = core_id * ROW_ITER * BLK_BATCHES + row_idx * BLK_BATCHES + tl.arange(0, BLK_BATCHES)
        for col_idx in range(0, COL_ITER):
            ...
            softplus_x = tl.where(gbeta * x <= threshold, (1 / gbeta) * tl.log(1 + tl.exp(gbeta * x)), x)
    ...
    #
    # JITFunction.run() re-does per call: make_backend(target), three driver queries,
    # argument binding, a string cache key (''.join(sig) + str(constexprs)) and a scan
    # of used_global_vals - about 20us of the ~283us host cost of a Triton launch on
    # this stack (measured: full path 31.99us vs pre-bound 11.93us in a warm loop).
    # Calling the CompiledKernel directly skips all of it.
    #
    # Correctness: Triton 3.2 keys its cache on arg type strings plus the attrs
    # descriptor. Probing this kernel over 5 shape variants (T = 3512/1464/127 tokens)
    # yields exactly one specialisation, and the key shows 14 divisibility markers -
    # one per pointer. Because every int arg is in do_not_specialize, ints do not
    # affect the key at all; only pointer dtype and 16-byte alignment do. The guard
    # below therefore covers exactly what Triton would have recomputed.
...
def gating_gather_clear_l2norm_qk(
    ...
    chain plus two ``l2norm_fwd`` calls, i.e. four launches. Measured on this stack a
    Triton launch costs ~300us of host time while these kernels run in under 20us, so
    the launch count is what decides the cost: host time per GDN layer tracked
    linearly with the number of Triton launches (2 launches -> 810us, 3 -> 1415us).
    Collapsing to one took the layer from 1085us of host time to 356us, below its
    661us of device time, which dropped the prep bubble from 582us to 163us.

    Gating is elementwise over tokens (only A_log/dt_bias are indexed by head, no
    reduction across tokens), so restricting it to this batch's tokens is bit-exact
    against computing the full extent and slicing - that is what lets it join a
    prefill-only launch.

    Returns ``(g, beta_output, initial_state, q_norm, k_norm)``.

    Callers must gate this on :func:`preamble_fusion_enabled`; it is only valid for a
    pure-prefill batch. The preconditions below hold for every GDN model we support,
    so a violation is a bug rather than a case to degrade silently.
    """
    batch, num_heads = a.shape
    if not (q.shape == k.shape and q.dtype == k.dtype):
        raise ValueError(
            f"gating_gather_clear_l2norm_qk: q/k must match, got {q.shape}/{q.dtype} "
            f"vs {k.shape}/{k.dtype}. Set VLLM_ASCEND_GDN_FUSE_CLEAR_L2NORM=0 to use "
            "the unfused path."
        )
    if not (ssm_state.numel() > 0 and ssm_state.is_contiguous()):
        raise ValueError(
            "gating_gather_clear_l2norm_qk: ssm_state must be non-empty and contiguous. "
            "Set VLLM_ASCEND_GDN_FUSE_CLEAR_L2NORM=0 to use the unfused path."
        )
    if not (a.is_contiguous() and b.is_contiguous() and a.shape == b.shape):
        raise ValueError(
            f"gating_gather_clear_l2norm_qk: a/b must be contiguous and same-shaped, "
            f"got {a.shape}/{b.shape}. Set VLLM_ASCEND_GDN_FUSE_CLEAR_L2NORM=0 to use "
            "the unfused path."
        )

    # The builder already normalized these once per step (see
    # gdn_attn_builder._build_non_spec_metadata). Re-doing it here would cost ~6 aten
    # dispatches per GDN layer for what is a no-op, so only convert if a caller hands
    ...
    if num_rows == 0 or inner_size == 0:
        raise ValueError(
            f"gating_gather_clear_l2norm_qk: empty state (rows={num_rows}, "
            f"inner={inner_size}); the caller should not reach here for a prefill batch."
        )
    if has_initial_state.numel() != num_rows:
        raise ValueError(
            f"gating_gather_clear_l2norm_qk: has_initial_state size mismatch: "
            f"expected {num_rows}, got {has_initial_state.numel()}"
        )

    # reshape also normalizes a non-contiguous input by copying, which the kernel's
    # flat indexing (X + rindex + N * row_idx) relies on. Passing q/k through directly
    # instead was tried and measured at ~1us/layer, so it is not worth trading that
    # implicit guarantee for an explicit contiguity precondition.
    ...
    if ck is None:
        ...
        # run() returns the CompiledKernel it used (jit.py:652), which is safer than
        # guessing from the cache dict when several specialisations coexist.
        if (
    ...
    # Pre-bound: mirrors the tail of JITFunction.run (jit.py:650) without its preamble.
    # Non-constexpr values in declaration order; launch_metadata and the enter/exit
    # hooks are None because nothing installs them here.
...
def _gating_gather_clear_l2norm_qk_kernel(
    ...
    if pid < N_GATHER:
        ...
        _gdn_gating_rows(
            ...
            gbeta, threshold, BLK_HEADS, BLK_BATCHES, ROW_ITER, pid - N_GATHER,
        )
